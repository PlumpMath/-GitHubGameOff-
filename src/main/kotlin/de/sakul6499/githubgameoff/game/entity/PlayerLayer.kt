...
object PlayerLayer : Layer("Player", 5, BackgroundLayer.v0, BackgroundLayer.v1) {
    ...
    internal val player: Player = Player(Vector2F(BackgroundLayer.getEndX() / 2, BackgroundLayer.getEndY() / 2))
...
//            if (coolDown <= 0) {
//                BulletLayer.bullets.add(BulletLayer.Bullet(GetMidPosition(), cross.copy(), BulletLayer.Bullet.BulletType.NORMAL_RED))
...
//        // Bullet cross
//        if (!cross.isNull(crossTolerance)) {
//            val mid = GetMidPosition()
//            renderImage(graphics, Images.getImage("cross"), mid.getRoundX() + cross.getRoundX(), mid.getRoundY() + cross.getRoundY(), size / 2, size / 2)
//        }
    }
}